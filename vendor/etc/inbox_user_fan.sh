...
function enable_mic(){
	...
	echo "[INBOX_FAN][USER]enable_mic after sys.asus.fan.mic changed" > /dev/kmsg
}
...
if [ "$dongle_type" == "1" ]; then
	...
	if [ "$mic_type_new" == "$mic_type" ];then
		echo "[INBOX_FAN][USER] mic_type_new=$mic_type_new mic_type=$mic_type " > /dev/kmsg
		exit 0
	else
		echo "[INBOX_FAN][USER]user fan set . 1st check mic_type=$mic_type; mic_type_new=$mic_type_new " > /dev/kmsg
	...
	if [ "$mic_type_new" == "$mic_type" ];then
		...
		echo "[INBOX_FAN][USER]user fan set . 2nd check mic_type=$mic_type; mic_type_new=$mic_type_new " > /dev/kmsg
		if [ "$micfansettings_type" == "1" ] && [ "$mic_type_new" == "1" ]; then
			enable_mic
			exit 0
		else
