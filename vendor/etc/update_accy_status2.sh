...
DT_PD_FW_VER=`getprop sys.asusfw.dt.pd_fwver`
DT_HUB1_FW_VER=`getprop sys.asusfw.dt.hub1_fwver`
DT_HUB2_FW_VER=`getprop sys.asusfw.dt.hub2_fwver`

pd_version=`getprop sys.dt.pd_fwver`
...
if [ "$type" == "3" ]; then
	echo "$LOG_TAG:[USB_ACCY] sys.asus.accy.fw_status2 = 000$dt_pd_status$hub1_update$hub2_update" > /dev/kmsg
	setprop sys.asus.accy.fw_status2 000"$dt_pd_status""$hub1_update""$hub2_update"
	setprop sys.dt.accy_statusupdate 0
else
	echo "$LOG_TAG:[USB_ACCY] DT is disconnect, ignore" > /dev/kmsg
	setprop sys.dt.accy_statusupdate 2
fi
