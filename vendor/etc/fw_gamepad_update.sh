...
echo "[Gamepad fw update] script entry.  get type=&type & ver first" > /dev/kmsg
retry=0
...
function fw_update(){
	...
	if [ "$type" == "left_handle" ]; then
		...
		result="$?"
		echo "[Gamepad fw update] fw_gamepad_update after update cmd ,result=$result." > /dev/kmsg
		if [ "$result" == 0 ]; then
			sleep 1
			/vendor/bin/gamepad_serialnum_get -a
			...
			echo "[Gamepad fw update] fw_gamepad_update,after update,the result is not 0,1,2, fw_ver=$fw_ver asus_fw_ver=$asus_fw_ver " > /dev/kmsg
			setprop sys.gamepad.left_fwupdate 2
		fi
