...
function enable_mic(){
	...
	echo "[DT_FAN][AUTO]enable_mic after sys.asus.fan.mic changed" > /dev/kmsg
}
...
function disable_mic(){
	...
	if [ "$fan_type" == "auto" ]; then
		...
		if [ "$thermal_type" == "0" ]; then
			...
			cat /sys/class/hwmon/Inbox_Fan/device/PWM
			echo "[DT_FAN][AUTO]thermal_type 0: default 1 (low)" > /dev/kmsg
		elif [ "$thermal_type" == "1" ]; then
			echo 1 > /sys/class/hwmon/Inbox_Fan/device/dt_thermal_type
			cat /sys/class/hwmon/Inbox_Fan/device/PWM
			echo "[DT_FAN][AUTO]thermal_type 1: low" > /dev/kmsg
		elif [ "$thermal_type" == "2" ]; then
			echo 2 > /sys/class/hwmon/Inbox_Fan/device/dt_thermal_type
			cat /sys/class/hwmon/Inbox_Fan/device/PWM
			echo "[DT_FAN][AUTO]thermal_type 2: mediun" > /dev/kmsg
		elif [ "$thermal_type" == "3" ]; then
			echo 3 > /sys/class/hwmon/Inbox_Fan/device/dt_thermal_type
			cat /sys/class/hwmon/Inbox_Fan/device/PWM
			echo "[DT_FAN][AUTO]thermal_type 3: high" > /dev/kmsg
		elif [ "$thermal_type" == "4" ]; then
			echo 4 > /sys/class/hwmon/Inbox_Fan/device/dt_thermal_type
			cat /sys/class/hwmon/Inbox_Fan/device/PWM
			echo "[DT_FAN][AUTO]thermal_type 4: turbo" > /dev/kmsg
		fi
	else
		echo "[DT_FAN][AUTO]thermal fan will not  be set auto" > /dev/kmsg
	fi
}
...
if [ "$dongle_type" == "3" ]; then
	...
	if [ "$mic_type_new" == "$mic_type" ];then
		echo "[DT_FAN][AUTO] mic_type_new=$mic_type_new mic_type=$mic_type" > /dev/kmsg
		exit 0
	else
		echo "[DT_FAN][AUTO]thermal fan set . 1st check mic_type=$mic_type; mic_type_new=$mic_type_new " > /dev/kmsg
	...
	if [ "$mic_type_new" == "$mic_type" ];then
		...
		echo "[DT_FAN][AUTO]thermal fan set . 2nd check mic_type=$mic_type; mic_type_new=$mic_type_new " > /dev/kmsg
		if [ "$micfansettings_type" == "1" ] && [ "$mic_type_new" == "1" ]; then
			enable_mic
			exit 0
		else
